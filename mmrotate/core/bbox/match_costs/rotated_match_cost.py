...
class GWDIoUCost:
    ...
    def __init__(self, iou_mode='giou', representation='xy_wh_r', weight=1.):
        self.weight = weight
        self.iou_mode = iou_mode
...
class KLDIoUCost:
    ...
    def __call__(self, pred, target, fun='log1p', tau=1.0, alpha=1.0, sqrt=True):
        xy_p, Sigma_p = pred
        xy_t, Sigma_t = target
        ...
        dxy = (xy_p[..., None, :2] - xy_t).unsqueeze(-1)
        xy_distance = 0.5 * dxy.permute(0, 2, 1).matmul(Sigma_p_inv[None, :, ...]).matmul(dxy).view(-1)
        # xy_distance = 0.5 * dxy.permute(0, 2, 1).matmul(Sigma_p_inv[:, None, :, ...]).matmul(dxy).view(-1)
        # xy_distance = 0.5 * Sigma_p_inv[:, None, ...].matmul(dxy.permute(0, 2, 1)).matmul(dxy).view(-1)

        whr_distance = 0.5 * Sigma_p_inv[:, None, :, ...].matmul(Sigma_t).diagonal(
            dim1=-2, dim2=-1).sum(dim=-1)
